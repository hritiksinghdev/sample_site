      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-10">
        ...
        <div className="lg:hidden flex gap-4 overflow-x-auto hide-scrollbar snap-x -mx-5 px-5 pb-2">
          {testimonials.map((t, i) => (
            <motion.div
              key={i}
              className="flex-shrink-0 w-[80vw] snap-center bg-card border border-border rounded-xl p-6"
              initial={{ opacity: 0, x: 20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: i * 0.1 }}
            >
              <svg className="w-6 h-6 text-gold/40 mb-4" viewBox="0 0 24 24" fill="currentColor">
              ...
              <p className="text-secondary text-[0.85rem] leading-relaxed font-light mb-5 italic">
                {t.text}
              </p>
              <div>
                <p className="text-primary text-sm font-medium">{t.author}</p>
                <p className="text-secondary/70 text-[0.65rem] mt-0.5">{t.location}</p>
              </div>
            </motion.div>
          ))}
        </div>
